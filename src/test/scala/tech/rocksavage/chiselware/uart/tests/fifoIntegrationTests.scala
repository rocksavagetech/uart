...
object fifoIntegrationTests {
    ...
    def txFifoOverflowTest(dut: Uart, params: UartParams): Unit = {
        ...
        println(
          s"Attempting to fill TX FIFO beyond capacity (${params.bufferSize} entries)"
        )
        for (i <- 0 until params.bufferSize + 3) {
            println(s"Writing TX data ${i + 1}/${params.bufferSize + 3}")
            writeAPB(
              dut.io.apb,
...
//      writeAPB(dut.io.apb, dut.registerMap.getAddressOfRegister("tx_load").get.U, true.B)
            ...
            if (i > params.bufferSize) {
                ...
                val hasOverflowError = (errorStatus) != 0
                ...
                assert(
                  hasOverflowError,
                  s"TX FIFO overflow error should be set but got error status $errorStatus"
                )
            }
        }
    ...
    def rxFifoOverflowTest(dut: Uart, params: UartParams): Unit = {
        ...
        writeAPB(
          dut.io.apb,
          dut.registerMap.getAddressOfRegister("rx_clockFreq").get.U,
          clockFreq.U
        )
        writeAPB(
          dut.io.apb,
          dut.registerMap.getAddressOfRegister("rx_baudRate").get.U,
          baudRate.U
        )
        writeAPB(
          dut.io.apb,
          dut.registerMap.getAddressOfRegister("rx_updateBaud").get.U,
          1.U
        )
        clock.step(20)
        ...
        println(
          s"Filling RX FIFO beyond capacity (${params.bufferSize} entries)"
        )
        ...
        for (i <- 0 until params.bufferSize + 2) {
            val char = ('A' + i).toChar
            println(
              s"Sending character $char (${i + 1}/${params.bufferSize + 2})"
            )
            ...
            if (i >= params.bufferSize) {
                ...
                assert(
                  hasError,
                  s"Error should be set but got error status $errorStatus"
                )
            }
        }
    ...
    def txFifoUnderflowTest(dut: Uart, params: UartParams): Unit = {
        ...
        println(
          s"Error status after attempting to load from empty FIFO: $errorStatus"
        )
        println(s"TX FIFO underflow bit set: $hasUnderflowError")

        assert(
          hasUnderflowError,
          s"TX FIFO underflow error should be set but got error status $errorStatus"
        )
    ...
    def rxFifoUnderflowTest(dut: Uart, params: UartParams): Unit = {
        ...
        assert(
          hasUnderflowError,
          s"RX FIFO underflow error should be set but got error status $errorStatus"
        )
    ...
    def errorClearingTest(dut: Uart, params: UartParams): Unit = {
        ...
        val txErrorStatus = readAPB(
          dut.io.apb,
          dut.registerMap.getAddressOfRegister("error").get.U
        )
        val txOverflowSet =
            (txErrorStatus & UartTxError.FifoOverflow.litValue) != 0
        println(
          s"TX FIFO overflow error set: $txOverflowSet (error=$txErrorStatus)"
        )
        ...
        val combinedErrorStatus = readAPB(
          dut.io.apb,
          dut.registerMap.getAddressOfRegister("error").get.U
        )
        ...
        val finalErrorStatus = readAPB(
          dut.io.apb,
          dut.registerMap.getAddressOfRegister("error").get.U
        )
        println(
          s"Final error status after clearing multiple errors: $finalErrorStatus"
        )
        assert(
          finalErrorStatus == 0,
          "All errors should be cleared with a single clearError operation"
        )
    }
    ...
    private def setupUart(
        ...
        println(
          s"Setting up UART with clockFreq=$clockFreq, baudRate=$baudRate"
        )

        // Configure TX
        writeAPB(
          apb,
          uart.registerMap.getAddressOfRegister("tx_clockFreq").get.U,
          clockFreq.U
        )
        writeAPB(
          apb,
          uart.registerMap.getAddressOfRegister("tx_baudRate").get.U,
          baudRate.U
        )
        writeAPB(
          apb,
          uart.registerMap.getAddressOfRegister("tx_updateBaud").get.U,
          true.B
        )
        writeAPB(
          apb,
        ...
        writeAPB(
          apb,
          uart.registerMap.getAddressOfRegister("rx_clockFreq").get.U,
          clockFreq.U
        )
        writeAPB(
          apb,
          uart.registerMap.getAddressOfRegister("rx_baudRate").get.U,
          baudRate.U
        )
        writeAPB(
          apb,
          uart.registerMap.getAddressOfRegister("rx_updateBaud").get.U,
          true.B
        )
        writeAPB(
          apb,
        ...
        writeAPB(
          apb,
          uart.registerMap.getAddressOfRegister("clearError").get.U,
          true.B
        )
        clock.step(2)
